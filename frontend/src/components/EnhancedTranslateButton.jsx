...
const EnhancedTranslateButton = ({ onLanguageChange }) => {
  const [isVisible, setIsVisible] = useState(false);
  const [currentLanguage, setCurrentLanguage] = useState('en');

  // Detect language changes from Google Translate
  useEffect(() => {
    const detectLanguageChange = () => {
      // Monitor Google Translate widget for language changes
    ...
    if (isVisible) {
      const cleanup = detectLanguageChange();
      return cleanup;
    }
  }, [isVisible, currentLanguage, onLanguageChange]);
  ...
  return (
    <div className="relative">
      <button onClick={handleClick} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
        {isVisible ? (
          <svg
            width="24"
            height="24"
      ...
      {isVisible && (
        <div className="absolute bottom-full right-0 mb-2 bg-white shadow-lg rounded-lg border p-3 min-w-[200px] z-50">
          <GoogleTranslate />
          {currentLanguage !== 'en' && (
            <div className="mt-2 text-xs text-gray-500">
              Current: {getLanguageName(currentLanguage)}
            </div>
          )}
        </div>
      )}
...
export default EnhancedTranslateButton;